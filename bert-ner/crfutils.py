...
class ConditionalRandomField(torch.nn.Module):
    ...
    def _input_likelihood(self, logits: torch.Tensor, mask: torch.BoolTensor) -> torch.Tensor:
        """
        Compute likelihood (sum over all possible tags).
        ...
        for i in range(1, sequence_length):
            ...
            # In valid positions (mask == True) we want to take the logsumexp over the current_tag
            # dimension of `inner`. Otherwise (mask == False) we want to retain the previous alpha.
